    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-[var(--color-dark-slate)]">Sectors</h1>
                    <p className="text-sm text-[var(--color-subtle-text)] mt-1">{sectors?.length ?? 0} sectors total</p>
                </div>
                <Link
                ...
                <table className="w-full text-sm">
                    <thead className="bg-[var(--color-primary-solid)]/50 border-b border-slate-800">
                        <tr>
                            ...
                            <th className="text-left px-4 py-3 text-[var(--color-placeholder)] font-medium">Mandal</th>
                    ...
                    <tbody>
                        {sectors?.map((sector) => (
                            <tr key={sector.id} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="px-4 py-3 text-[var(--color-placeholder)] font-mono text-xs">{sector.code}</td>
                                <td className="px-4 py-3 text-[var(--color-placeholder)]">{(sector.mandals as any)?.name}</td>
